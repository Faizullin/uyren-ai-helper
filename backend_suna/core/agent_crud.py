...
from .utils.limits_checker import check_agent_count_limit
...
async def update_agent(
    ...
    if config.ENV_MODE == EnvMode.STAGING:
        print(
            f"[DEBUG] update_agent: Received icon fields - icon_name={agent_data.icon_name}, icon_color={agent_data.icon_color}, icon_background={agent_data.icon_background}")

    client = await utils.db.client

    try:
        existing_agent = await client.table('agents').select('*').eq("agent_id", agent_id).eq("account_id",
                                                                                              user_id).maybe_single().execute()
        ...
        if agent_data.is_default is not None:
            ...
            if agent_data.is_default:
                await client.table('agents').update({"is_default": False}).eq("account_id", user_id).eq("is_default",
                                                                                                        True).neq(
        ...
        if config.ENV_MODE == EnvMode.STAGING:
            print(
                f"[DEBUG] update_agent: Prepared update_data with icon fields - icon_name={update_data.get('icon_name')}, icon_color={update_data.get('icon_color')}, icon_background={update_data.get('icon_background')}")

        current_system_prompt = agent_data.system_prompt if agent_data.system_prompt is not None else current_version_data.get(
            'system_prompt', '')
        ...
        current_agentpress_tools = agent_data.agentpress_tools if agent_data.agentpress_tools is not None else current_version_data.get(
            'agentpress_tools', {})
        ...
        if update_data:
            try:
                ...
                update_result = await client.table('agents').update(update_data).eq("agent_id", agent_id).eq(
                    "account_id", user_id).execute()
                ...
                if config.ENV_MODE == EnvMode.STAGING:
                    if update_result.data:
                        ...
                        print(
                            f"[DEBUG] update_agent DB UPDATE RESULT: {update_result.data[0] if update_result.data else 'No data'}")
                    else:
        ...
        updated_agent = await client.table('agents').select('*').eq("agent_id", agent_id).eq("account_id",
                                                                                             user_id).maybe_single().execute()
        ...
        print(
            f"[DEBUG] update_agent AFTER UPDATE FETCH: icon_name={agent.get('icon_name')}, icon_color={agent.get('icon_color')}, icon_background={agent.get('icon_background')}")
...
async def delete_agent(agent_id: str, user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    ...
    try:
        ...
        try:
            ...
            triggers_result = await client.table('agent_triggers').select('trigger_id').eq('agent_id',
                                                                                           agent_id).execute()
...
async def get_agents(
        ...
        content_type: Optional[str] = Query(None,
                                            description="Content type filter: 'agents', 'templates', or None for agents only")
):
    try:
...
async def create_agent(
    ...
    try:
        ...
        if config.ENV_MODE == EnvMode.STAGING:
            print(
                f"[DEBUG] create_agent: Creating with icon_name={insert_data.get('icon_name')}, icon_color={insert_data.get('icon_color')}, icon_background={insert_data.get('icon_background')}")
...
@router.post("/agents/generate-icon", response_model=AgentIconGenerationResponse, summary="Generate Agent Icon",
