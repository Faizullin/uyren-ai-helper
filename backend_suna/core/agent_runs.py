...
from .api_models import AgentStartRequest, AgentVersionResponse, ThreadAgentResponse, \
    InitiateAgentResponse
from .core_utils import (
    _get_version_service
)
from .utils.limits_checker import check_agent_run_limit, check_project_count_limit
...
async def start_agent(
    ...
    logger.debug(
        f"Starting new agent for thread: {thread_id} with config: model={model_name} (Instance: {utils.instance_id})")
    client = await utils.db.client

    thread_result = await client.table('threads').select('project_id', 'account_id', 'metadata').eq('thread_id',
                                                                                                    thread_id).execute()
    ...
    if not agent_data:
        ...
        default_agent = await client.table('agents').select('agent_id').eq('account_id', account_id).eq('is_default',
                                                                                                        True).maybe_single().execute()

        if default_agent.data:
            agent_data = await loader.load_agent(default_agent.data['agent_id'], user_id, load_config=True)
            logger.debug(
                f"Using default agent: {agent_data.name} ({agent_data.agent_id}) version {agent_data.version_name}")
        else:
    ...
    if config.ENV_MODE != EnvMode.LOCAL:
        ...
        if not limit_check['can_start']:
            ...
            logger.warning(
                f"Agent run limit exceeded for account {account_id}: {limit_check['running_count']} running agents")
...
async def get_agent_runs(thread_id: str, user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    ...
    agent_runs = await client.table('agent_runs').select(
        'id, thread_id, status, started_at, completed_at, error, created_at, updated_at').eq("thread_id",
                                                                                             thread_id).order(
...
@router.get("/thread/{thread_id}/agent", response_model=ThreadAgentResponse, summary="Get Thread Agent",
            operation_id="get_thread_agent")
async def get_thread_agent(thread_id: str, user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    ...
    try:
        ...
        recent_agent_result = await client.table('agent_runs').select('agent_id', 'agent_version_id').eq('thread_id',
                                                                                                         thread_id).not_.is_(
            'agent_id', 'null').order('created_at', desc=True).limit(1).execute()
        ...
        agent_result = await client.table('agents').select('*').eq('agent_id', effective_agent_id).eq('account_id',
                                                                                                      account_id).execute()
                    ...
                    if queue_item["type"] == "new_response":
                        ...
                        if new_responses_json:
                            ...
                            for response in new_responses:
                                ...
                                if response.get('type') == 'status' and response.get('status') in ['completed',
                                                                                                   'failed', 'stopped']:
                                    logger.debug(
...
@router.post("/agent/initiate", response_model=InitiateAgentResponse, summary="Initiate Agent Session",
...
async def initiate_agent_with_files(
    ...
    logger.debug(
        f"Initiating new agent with prompt and {len(files)} files (Instance: {utils.instance_id}), model: {model_name}")
    ...
    if agent_id:
        ...
        # Load default agent
        logger.debug("[AGENT INITIATE] Loading default agent")
        default_agent = await client.table('agents').select('agent_id').eq('account_id', account_id).eq('is_default',
                                                                                                        True).maybe_single().execute()
    ...
    if config.ENV_MODE != EnvMode.LOCAL:
        ...
        if not project_limit_check['can_create']:
            ...
            logger.warning(
                f"Project limit exceeded for account {account_id}: {project_limit_check['current_count']}/{project_limit_check['limit']} projects")
                ...
                if file.filename:
                    try:
                        ...
                        try:
                            ...
                            logger.error(f"Error during sandbox upload call for {safe_filename}: {str(upload_error)}",
                                         exc_info=True)
                        ...
                        if upload_successful:
                            try:
                                ...
                                logger.error(f"Error verifying file {safe_filename} after upload: {str(verify_error)}",
                                             exc_info=True)
                                failed_uploads.append(safe_filename)
                        else:
                            failed_uploads.append(safe_filename)
