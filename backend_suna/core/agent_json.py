...
class JsonImportService:
    ...
    async def _build_agent_config_from_json(
        ...
        for req in requirements:
            if req.custom_type == 'composio':
                ...
                custom_config = request.custom_mcp_configs.get(
                    req.qualified_name) if request.custom_mcp_configs else None
                if custom_config:
                    mcp_config = {
                        'name': req.display_name,
    ...
    async def _create_initial_version(
        ...
        try:
            logger.debug(
                f"Creating initial version for JSON imported agent {agent_id} with system_prompt: {system_prompt[:100]}...")
            ...
            if agent_check.data and agent_check.data[0].get('current_version_id'):
                logger.debug(
                    f"Agent {agent_id} current_version_id updated to: {agent_check.data[0]['current_version_id']}")
            else:
...
async def export_agent(agent_id: str, user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    ...
    try:
        ...
        if agent.get('current_version_id'):
            version_result = await client.table('agent_versions').select('*').eq('version_id',
                                                                                 agent['current_version_id']).execute()
        ...
        if agent.get('metadata'):
            export_metadata = {k: v for k, v in agent['metadata'].items()
                               if k not in ['is_suna_default', 'centrally_managed', 'installation_date',
                                            'last_central_update']}
...
@router.post("/agents/json/analyze", response_model=JsonAnalysisResponse, summary="Analyze Agent JSON",
             operation_id="analyze_agent_json")
...
@router.post("/agents/json/import", response_model=JsonImportResponse, summary="Import Agent from JSON",
             operation_id="import_agent_json")
