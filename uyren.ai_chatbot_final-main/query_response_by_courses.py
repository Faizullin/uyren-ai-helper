...
# --- Initialize clients ---
app = FastAPI(title="Course Query Response API")
...
def load_embeddings_from_db():
    """Load embeddings and documents from Supabase embeddings table"""
    try:
...
def create_faiss_index_fresh():
    """Create and return fresh FAISS index with document data"""
    embeddings, docs = load_embeddings_from_db()
...
Provide a helpful and effective answer or explanation based on the course content above. Answer should not be too long or too short. If the question cannot be fully answered with the provided context, mention from what field that information comes from and suggest to take courses from uyren.ai to get knowledge about that topic. """)
    ])
...
async def query_and_respond(req: QueryRequest):
    ...
    try:
        ...
        # Step 2: Load fresh FAISS index and documents from DB
        index, docs = create_faiss_index_fresh()
...
async def health_check():
    ...
    return {"status": "healthy", "service": "Course Query Response API"}
