...
class FileNameValidator:
    ...
    @classmethod
    def validate_name(
        cls, name: str, item_type: str = "file"
        ...
        if len(name) > 255:
            return (
                False,
                f"{item_type.capitalize()} name is too long (max 255 characters)",
            )
        ...
        if re.search(cls.ILLEGAL_CHARS, name):
            ...
            return (
                False,
                f"{item_type.capitalize()} name contains illegal characters: {', '.join(set(illegal_found))}",
            )
        ...
        if (
            ...
            return (
                False,
                f"{item_type.capitalize()} name cannot start or end with dots or spaces",
            )
        ...
        if base_name in cls.RESERVED_NAMES:
            return (
                False,
                f"{item_type.capitalize()} name '{base_name}' is reserved by the system",
            )
    ...
    def generate_unique_name(
        cls, base_name: str, existing_names: list[str], item_type: str = "file"
    ) -> str:
        """
