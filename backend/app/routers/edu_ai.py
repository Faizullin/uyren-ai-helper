...
# Initialize services for edu_ai module
edu_ai_storage = get_storage_service(prefix="edu-ai")
...
async def upload_course_material(
    ...
    **Note:** Requires pgvector extension for full functionality
    ...
    try:
        ...
        # Extract text from file using file processor
        text_content = file_processor.extract_text_content(
            content, file.filename or "document", file.content_type or "text/plain"
        )

        # Save file to storage for future reference
        file_path = await edu_ai_storage.save_file(
            file_content=content,
            filename=file.filename or f"{title}.txt",
            mime_type=file.content_type or "application/octet-stream",
            path_parts=[str(assignment_id), "materials"],
        )
        ...
        material = CourseMaterial(
            ...
            file_url=file_path,
            # embedding=None,  # TODO: Generate embedding when pgvector is enabled
        )
