"""Knowledge Base file processor service."""
...
class KnowledgeBaseFileProcessor:
    """Process files for knowledge base storage and summarization."""
    ...
    def __init__(self):
        self.client = (
            AsyncOpenAI(api_key=settings.DEFAULT_OPENAI_API_KEY)
            if settings.DEFAULT_OPENAI_API_KEY
            else None
        )

    def sanitize_filename(self, filename: str) -> str:
        """Sanitize filename for storage - remove/replace invalid characters."""
    ...
    def _is_likely_text_file(self, file_content: bytes) -> bool:
        ...
        try:
            ...
            if detected.get("confidence", 0) > 0.7:
                ...
                printable_ratio = len(
                    [c for c in decoded if c.isprintable() or c.isspace()]
                ) / len(decoded)
    ...
    async def generate_summary(self, content: str, filename: str) -> str:
        ...
        try:
            if not self.client:
                logger.warning("OpenAI client not configured, using fallback summary")
                return self._create_fallback_summary(content, filename)

            # Model priority: GPT-4o-mini → GPT-3.5-turbo
            models = [
                ("gpt-4o-mini", 128_000),  # 128K context
                ("gpt-3.5-turbo", 16_000),  # 16K context (fallback)
            ]
            ...
            for model_name, context_limit in models:
                try:
                    ...
                    if estimated_tokens <= usable_context:
                        ...
                        processed_content = self._smart_chunk_content(
                            content, usable_context * 4
...
Keep it under 200 words and make it actionable for context injection."""
                    ...
                    if summary:
                        logger.info(
                            f"Summary generated successfully using {model_name}"
                        )
                        return summary
    ...
    def _smart_chunk_content(self, content: str, max_chars: int) -> str:
        ...
        if remaining_chars > 0:
            ...
            for line in lines:
                ...
                is_important = (
                    line_stripped.isupper()  # Headers
                    or line_stripped.startswith(("•", "-", "*", "1.", "2."))  # Lists
                    or any(
                    ...
                    or len(line_stripped) < 100  # Short lines often important
                )
    ...
    def _create_fallback_summary(self, content: str, filename: str) -> str:
        ...
        return f"This {content_type} '{filename}' contains {len(content):,} characters across {len(non_empty_lines)} lines. Preview: {preview_text} This file would be useful for understanding the specific content and context it provides."
    ...
    async def process_file(
        ...
        """
        Process a file: extract content, generate summary.

        Note: This does NOT save to database or storage - that's done by the caller.
        This only processes the file and returns metadata.
        """
        try:
            ...
            is_text_based = (
                ...
                or mime_type in ["application/json", "application/xml", "text/xml"]
                or self._is_likely_text_file(file_content)
            )
            ...
            content = self.extract_text_content(file_content, filename, mime_type)
            if not content:
            ...
            summary = await self.generate_summary(content, filename)
